...
def master(X,y,w,N,K,M,threads,timelimit):
    global UBlist
    t = np.genfromtxt(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UBlist[0]).replace('.','p')+'/t.csv',delimiter=';',dtype=float)
    for UB_ in UBlist[1:]:
        t_ = np.genfromtxt(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB_).replace('.','p')+'/t.csv',delimiter=';',dtype=float)
        if len(t_.shape) == 1:
    ...
    if m.status == GRB.Status.OPTIMAL or m.status == GRB.Status.TIME_LIMIT and m.SolCount > 0:
        with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/ModelStatus.txt','a') as f:
            f.write('Master problem: ')
            if m.status == GRB.Status.OPTIMAL:
                f.write('OPTIMAL\n')
            elif m.status == GRB.Status.TIME_LIMIT:
                f.write('TIME_LIMIT\n')
        with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/Runtimes.txt','a') as f:
            f.write('Master problem: ')
        ...
        np.savetxt(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/q.csv',qnew,delimiter=';',fmt='%i')
        np.savetxt(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/y.csv',ynew,delimiter=';',fmt='%i')
        with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/ObjectiveValue_master.csv','a') as f:
...
def masterUB(UB):
    ...
    # Process results
    if m.status == GRB.Status.OPTIMAL or m.status == GRB.Status.TIME_LIMIT and m.SolCount > 0:
        with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')+'/ModelStatus.txt','a') as f:
            f.write('Master problem: ')
            if m.status == GRB.Status.OPTIMAL:
                f.write('OPTIMAL\n')
            elif m.status == GRB.Status.TIME_LIMIT:
                f.write('TIME_LIMIT\n')
        with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')+'/Runtimes.txt','a') as f:
            f.write('Master problem: ')
            f.write(str(time.time()-starttime)+'\n')
        for var in m.getVars():
            try:
        ...
        np.savetxt(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')+'/q.csv',qnew,delimiter=';',fmt='%i')
        np.savetxt(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')+'/y.csv',ynew,delimiter=';',fmt='%i')
        with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')+'/ObjectiveValue_master.csv','a') as f:
            f.write(str(m.getObjective().getValue())+'\n')
...
def subproblem(a):
    ...
    with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')+'/Messages.txt','a') as f:
    ...
    pselection = np.genfromtxt(runfolder+'/Pselect_'+str(UB).replace('.','p')+'_'+str(iter)+'.csv',delimiter=',',dtype=int)
    ...
    if initializing == 0:
        s_prev = np.genfromtxt(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UBlist[0]).replace('.','p')+'/s.csv',delimiter=';',dtype=float)
        if len(s_prev.shape) == 1:
            s_prev = np.expand_dims(s_prev,axis=0)

        for UB_ in UBlist[1:]:
            s_prev_ = np.genfromtxt(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB_).replace('.','p')+'/s.csv',delimiter=';',dtype=float)
    ...
    if m.status == GRB.Status.OPTIMAL or m.status == GRB.Status.TIME_LIMIT and m.SolCount > 0:
        ...
        snew = np.zeros((P_),dtype=int)
        snew_subset = np.empty((P),dtype=int)
        for var in m.getVars():
            splitname = var.VarName.split('[')
            if ',' in splitname[1]:
                splitname = [splitname[0],splitname[1].split(',')[0],splitname[1].split(',')[1]]
                ...
                splitname = [splitname[0],splitname[1].split(']')[0]]
            if splitname[0] == 's':
                snew_subset[int(splitname[1])] = var.X
        snew[pselection[np.squeeze(np.where(snew_subset==1))]] = 1

        tnew = np.floor(np.matmul(X_,snew)/np.sum(snew)).astype(int)
        X_subset = X_[:,pselection]
        tnew_subset = np.floor(np.matmul(X_subset,snew_subset)/np.sum(snew_subset)).astype(int)
        with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')+'/t_temp.csv','a') as f:
    ...
    result['s'] = snew
    result['t'] = tnew
    result['s_subset'] = snew_subset
    result['t_subset'] = tnew_subset
...
def process_results(results_coll):
    ...
    for UB_fl in UBlist_stillrunning:
        with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB_fl).replace('.','p')+'/Messages.txt','a') as f:
        ...
        for k,v in results_coll[UB_fl].items():
            ...
            with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')+'/Runtimes.txt','a') as f:
                f.write('Sub problem: ')
                f.write(v['runtime'])
            with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')+'/t.csv','a') as f:
                f.write(';'.join(v['t'].astype(str))+'\n')
            with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')+'/t_subset.csv','a') as f:
                f.write(';'.join(v['t_subset'].astype(str))+'\n')
            with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')+'/s.csv','a') as f:
                f.write(';'.join(v['s'].astype(str))+'\n')
            with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')+'/s_subset.csv','a') as f:
                f.write(';'.join(v['s_subset'].astype(str))+'\n')
            with open(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')+'/ObjectiveValue_sub.csv','a') as f:
                f.write(v['obj'])
...
def generate_firstPool(X,y,w,N,P,Nsub,UBlist):
    ...
    Nselect = np.random.choice(cases,Nsub,replace=False).tolist()
    ...
    p_select = np.sort(np.random.choice(range(P),Psub_,replace=False).tolist())
    np.savetxt(runfolder+'/Pselect_'+str(0.005).replace('.','p')+'_0.csv',p_select,delimiter=',',fmt='%i')
    ...
    for UB_frac in UBlist:
        ...
        p_select = np.sort(np.random.choice(range(P),Psub_,replace=False).tolist())
        np.savetxt(runfolder+'/Pselect_'+str(UB_frac).replace('.','p')+'_'+str(iter)+'.csv',p_select,delimiter=',',fmt='%i')
    ...
    os.system('/bin/rm -r '+runfolder+'/Pselect_*')
...
def add_rules(X,y,w,N,P,Nsub):
    ...
    for UB in UBlist_stillrunning:
        ...
        if len(fn) > Nsub:
            ...
            fn_select = np.random.choice(fn,int(np.floor(Nsub)),replace=False).tolist()
            ...
            p_select = np.sort(np.random.choice(range(P),Psub_,replace=False).tolist())
            np.savetxt(runfolder+'/Pselect_'+str(UB).replace('.','p')+'_'+str(iter)+'.csv',p_select,delimiter=',',fmt='%i')
...
if __name__ == "__main__":
    ...
    Psub_ = int(sys.argv[10])
    K = int(sys.argv[11])
    M = int(sys.argv[12])

    runfolder = runfilesfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)
    ...
    if not os.path.isdir(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)):
        os.system('mkdir '+resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_))
    for UB in UBlist:
        if not os.path.isdir(resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p')):
            os.system('mkdir '+resultfolder+'/'+dataset+'/K'+str(K)+'_M'+str(M)+'_time'+str(timelimit)+'_threads'+str(threads)+'_Nsub'+str(Nsub_)+'_Psub'+str(Psub_)+'/UB'+str(UB).replace('.','p'))
